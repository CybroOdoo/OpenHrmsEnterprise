...
def zksetuser(self, uid, userid, name, password, role):
    ...
    command_string = pack('sxs8s28ss7sx8s16s', chr(uid), chr(role),
                          password, name, chr(1), '', userid, '')
    chksum = 0
    ...
    try:
        ...
        if len(self.userdata) > 0:
            ...
            while len(userdata) > 72:
                ...
                password = unicode(password.strip('\x00|\x01\x10x'),
                                   errors='ignore')
                userid = unicode(userid.strip('\x00|\x01\x10x'),
                                 errors='ignore')
                name = name.split('\x00', 1)[0]
                if name.strip() == "":
                    name = uid
                users[uid] = (userid, name, int(role.encode("hex"), 16),
                              password)
