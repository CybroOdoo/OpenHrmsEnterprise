...
class HrCustody(models.Model):
    ...
    # smart button action for returning the view of all custody contracts
    ...
    def custody_view(self):
        for each1 in self:
            ...
            view_id = self.env.ref('ent_hr_custody.hr_custody_form_view').id
            if custody_ids:
    ...
    # smart button action for returning the view of all custody contracts
    # that are in approved state, related to the current employee
    ...
    def equipment_view(self):
        for each1 in self:
            ...
            view_id = self.env.\
                ref('ent_hr_custody.custody_custody_form_view').id
            if equipment_ids:
