...
class EmployeeVerificationPortal(CustomerPortal):
    ...
    def _prepare_home_portal_values(self, counters):
        ...
        values['verification_count'] = request.env[
    ...
    def portal_my_records(self, page=1, date_begin=None, date_end=None,
        ...
        return request.render("ent_employee_background.portal_my_records",
                              values)

    @http.route(['/my/details/<int:order>'], type='http', auth="user",
                website=True)
    def portal_record_page(self, order=None, access_token=None, **kw):
        ...
        return request.render("ent_employee_background.portal_record_page",
                              values)

    @http.route(['/my/verification/submit'], type='http', auth="user", website=True,
                csrf=False)
    def portal_submit_verification(self, **kw):
        """Fetch and load attached file in the model employee_verification"""
        employee = (request.env['employee.verification'].sudo().browse
                    (int(kw.get('employee_token', 0))))
        if kw['description'] or kw.get('attachment', False):
            if kw['description']:
                employee.write({
                    'description_by_agency': str(kw['description'])
                })
            ...
            return (request.render
                    ("ent_employee_background.portal_record_completed", values))
        else:
