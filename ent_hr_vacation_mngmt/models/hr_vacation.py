...
class HrLeaveRequest(models.Model):
    ...
    overlapping_leaves = fields.Many2many('hr.leave',
                                          compute='get_overlapping_leaves',
                                          string='Overlapping Leaves',
                                          help="Overlapping leaves")
    pending_tasks = fields.One2many('pending.task', 'leave_id',
                                    string='Pending Tasks',
                                    help="Pending tasks")
    holiday_managers = fields.Many2many('res.users',
                                        compute='get_hr_holiday_managers',
                                        help="Holiday managers")
    flight_ticket = fields.One2many('hr.flight.ticket', 'leave_id',
                                    string='Flight Ticket',
                                    help="Flight ticket")
    # Commented for odoo 14 compatibility double_validation = fields.Boolean(
    ...
    # def task(self):
    #     emp = self.employee_ids.user_id
    #     record_ids = self.env['project.task'].search(
    #         [('user_ids.id', 'in', self.employee_ids.user_id.ids)])
    #     print(record_ids, '433e5')
    #     #     domain = {'domain': {'pending_tasks': [
    #     #         (self.pending_tasks.name, '=', i.name)]}}
    #     #     print(domain)
    #     # return domain
    #     #     self.pending_tasks.name = i.name
    #     #     print("1234567890-")
    ...
    #     #     print(record.name)
    #     #     record.write({
    #     #         self.pending_tasks.name: record.name,
    #     #     })
    ...
    def get_overlapping_leaves(self):
        if self.date_from and self.date_to:
            ...
            for leave in other_leaves:
                ...
                leave_dtes = [str(frm_dte + timedelta(days=i)) for i in
                              range(r)]
                if set(leave_dtes).intersection(set(leave_dates)):
                    overlap_leaves.append(leave.id)
            print("Overlap:", overlap_leaves)
    ...
    def action_approve(self):
        ...
        for holiday in self:
            ...
            if holiday.pending_tasks:
                ...
                # if holiday.double_validation: return holiday.write({
                # 'state': 'validate1', 'manager_id': manager.id if manager
                # else False}) else:
    ...
    def book_ticket(self):
        ...
        return {
            ...
            'view_id': self.env.ref(
                'ent_hr_vacation_mngmt.view_hr_book_flight_ticket_form').id,
    ...
    def get_hr_holiday_managers(self):
        self.holiday_managers = self.env.ref(
    ...
    def send_leave_reminder(self):
        ...
        mail_template = self.env.ref(
            'ent_hr_vacation_mngmt.email_template_hr_leave_reminder_mail')
...
class PendingTask(models.Model):
    ...
    name = fields.Many2one('project.task', string='Task', required=True)
    # domain="[('user_ids.id', '=','leave_id.employee_ids.ids.user_id.id')]")
    ...
    unavailable_employee = fields.Many2many('hr.employee',
                                            string='Unavailable Employees',
                                            help="unavailable employee",
...
class ResConfigSettings(models.TransientModel):
    ...
    leave_reminder = fields.Boolean(string='Leave Reminder Email',
                                    help="Send leave remainder emails to hr "
                                         "managers")
    reminder_day_before = fields.Integer(string='Reminder Day Before')
    default_expense_account = fields.Many2one('account.account',
                                              string='Travel Expense Account',
                                              default_model='hr.leave')

    default_leave_salary = fields.Selection([('0', 'Basic'), ('1', 'Gross')],
                                            string='Leave Salary',
                                            default_model='hr.leave')
    ...
    def set_values(self):
        ...
        self.env['ir.config_parameter'].sudo().set_param('leave_reminder',
                                                         self.leave_reminder)
        self.env['ir.config_parameter'].sudo().set_param('reminder_day_before',
                                                         self.reminder_day_before)
        self.env['ir.config_parameter'].sudo().set_param(
            'travel_expense_account', self.default_expense_account.id)

        self.env['ir.config_parameter'].sudo().set_param('default_leave_salary',
                                                         self.default_leave_salary)
